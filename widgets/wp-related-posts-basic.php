 ...
 * @see WP_Widget
 */
class WP_Widget_Related_Posts_with_Relevanssi extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'classname' => 'WP_Widget_Related_Posts_with_Relevanssi',
			'description' => __( 'Related posts generated with Relevanssi, SearchWP, Elasticpress or default WordPress search engine.' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'WP_Widget_Related_Posts_with_Relevanssi', __( 'Related Posts with Relevanssi' ), $widget_ops );
		$this->alt_option_name = 'widget_related_entries';

		//$this->name = 
		$this->name = __("Relevant Related Posts");
	}
	...
	public function widget( $args, $instance ) {
		...
		echo $related_posts_with_relevanssi_init->related_posts_shortcode($instance);
...
?>
		...
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" placeholder="<?php _e( 'Ex: Related', 'related-posts-with-relevanssi' ); ?>"/></p>

		<?php if(count($ptypes)==1){ ?>
		<p><label for="<?php echo $this->get_field_id( 'keyword' ); ?>"><?php _e( 'Keyword:' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'keyword' ); ?>" name="<?php echo $this->get_field_name( 'keyword' ); ?>" type="text" value="<?php echo $keyword; ?>"  placeholder="<?php _e( 'Ex: test', 'related-posts-with-relevanssi' ); ?>"/></p>
		<p><?php _e("Install", 'related-posts-with-relevanssi'); ?> <a href="<?php echo $related_posts_with_relevanssi_init->ps->showExtUrl(); ?>" target="_blank"><?php echo $related_posts_with_relevanssi_init->conf['plugins']['types_and_automation']; ?></a> <?php _e("plugin to enable suggestions by title and keywords of current post", 'related-posts-with-relevanssi'); ?>.</p>
		<?php }else{ ?>
		...
		<select class="widefat" id="<?php echo $this->get_field_id( 'key_source' ); ?>" name="<?php echo $this->get_field_name( 'key_source' ); ?>">
			<?php foreach($related_posts_with_relevanssi_init->conf['key_source'] as $option=>$label){ ?>
		...
		<select class="widefat" id="<?php echo $this->get_field_id( 'style' ); ?>" name="<?php echo $this->get_field_name( 'style' ); ?>">
			<?php foreach($related_posts_with_relevanssi_init->conf['style'] as $option=>$label){ ?>
		...
		<select class="widefat" id="<?php echo $this->get_field_id( 'order' ); ?>" name="<?php echo $this->get_field_name( 'order' ); ?>">
			<?php foreach($related_posts_with_relevanssi_init->conf['order'] as $option=>$label){ ?>
		...
		<input class="widefat" id="<?php echo $this->get_field_id( 'width' ); ?>" name="<?php echo $this->get_field_name( 'width' ); ?>" type="text" value="<?php echo $width; ?>" placeholder="<?php _e( 'Ex: 100%', 'related-posts-with-relevanssi' ); ?>"/></p>
		...
		<select class="widefat" id="<?php echo $this->get_field_id( 'align' ); ?>" name="<?php echo $this->get_field_name( 'align' ); ?>">
			<?php foreach($related_posts_with_relevanssi_init->conf['align'] as $option=>$label){ ?>
		...
		<?php if(count($ptypes)==1){ ?>
			<p><?php _e("Install", 'related-posts-with-relevanssi'); ?> <a href="<?php echo $related_posts_with_relevanssi_init->ps->showExtUrl(); ?>" target="_blank"><?php echo $related_posts_with_relevanssi_init->conf['plugins']['types_and_automation']; ?></a> <?php _e("plugin to enable extended post types including products", 'related-posts-with-relevanssi'); ?>.</p>
		<?php } ?>
		<p>&nbsp;</p>
		...
		<textarea class="widefat" id="<?php echo $this->get_field_id( 'include' ); ?>" name="<?php echo $this->get_field_name( 'include' ); ?>" placeholder="<?php _e( 'Ex: 3, 12, 33', 'related-posts-with-relevanssi' ); ?>"><?php echo $include; ?></textarea></p>

		<p><label for="<?php echo $this->get_field_id( 'exclude' ); ?>"><?php _e( 'Exclude:' ); ?></label>
		<textarea class="widefat" id="<?php echo $this->get_field_id( 'exclude' ); ?>" name="<?php echo $this->get_field_name( 'exclude' ); ?>" placeholder="<?php _e( 'Ex: 3, 12, 33', 'related-posts-with-relevanssi' ); ?>"><?php echo $exclude; ?></textarea></p>
...
add_action( 'widgets_init', function(){
	register_widget( 'WP_Widget_Related_Posts_with_Relevanssi' );
});
